...
Ответь ТОЛЬКО одним JSON-объектом, без текста до и после:
{
  ...
  "type": {"name": "типаж, например ingénue / soft gamine / soft natural / classic", "features": "1–2 предложения о чертах и что это значит для одежды"},
  ...
  "hair": "совет по цвету волос, одно предложение",
  "makeup": "совет по макияжу, 1–2 предложения",
  "tips": ["3–4 коротких конкретных совета по одежде"],
...
export function outfitsPrompt(items, profile, req = {}) {
  ...
  const prompt = `Ты мой персональный стилист. Собери 3 разных образа ТОЛЬКО из вещей моего гардероба ниже. Отвечай по-русски.
...
Ответь ТОЛЬКО одним JSON-объектом, без текста до и после:
{"outfits": [{"title": "название образа, 2–4 слова", "items": ["v1", "v5"], "why": "1–2 коротких предложения, почему это мне идёт"}], "tip": "какой вещи не хватает для этого запроса, или пустая строка"}`;
  return { prompt, map };
}
